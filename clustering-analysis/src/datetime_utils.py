"""
Конвертация года, месяца, дня, часа, минуты, секунды в десятичный год и обратно
"""
...
def checkDateTime(dateTime):
    """ Проверка, что час != 24, минута != 60, секунда != 60 """
    ...
    if isinstance(YR, (float, int)):
        ...
        # Устанавливаем все значения ниже нуля в ноль
        sel = SC < 0
        SC[sel] = 0
        sel = MN < 0
        MN[sel] = 0
        sel = HR < 0
        HR[sel] = 0        
        # Устанавливаем 60 в ноль и 24 в 23:59:59.99
        sel = abs(SC - 60) < 1e-6
        SC[sel] = 0
        MN[sel] = MN[sel] + 1 
        sel = 60 - MN < 1e-6
...
#------------------------------------------------------------------------------ 
#                        Конвертации даты и времени
#------------------------------------------------------------------------------
def dateTime2decYr(datetime_in, **kwargs):
    """
    Входные данные: datetime_in = массив, содержащий столбцы времени (год - секунда)
                   Выходные данные: дата в десятичном году
    """
    try:
        ...
        error_msg = "Массив datetime недействителен - %s; проверьте, корректны ли дата и время, например, нет ли SC > 60.." % datetime_in
        raise ValueError(error_msg)
    time_sc = o_dt.hour * 3600 + o_dt.minute * 60 + o_dt.second
    # Получаем номер дня в текущем году от 0 до 364 и добавляем время в секундах
    ...
    # Десятичный год = текущий год + доля дня/времени (в десятичном годе)
    return o_dt.year + year_fraction

def decYr2datetime(decimalYear):
    """
    Конвертация десятичного года в год/месяц/день...
    """
    ...
    if decDay >= 0 and decDay <= 31:
        ...
        print('Неверный ввод десятичного года')
    hour   = np.floor(rest * 24)
    rest   = 24 * rest - hour
    minute = np.floor(rest * 60)
    rest   = 60 * rest - minute
    second = rest * 60     
    if day == 0: # для целых десятичных лет
        day = 1
    try:  
